    ...
    const options = {
      renderNode: {
        ...
        [INLINES.HYPERLINK]: (node) => {
          return <a href={node.data.uri} target={`${node.data.uri.startsWith("https://mile-portfolio.netlify.app") ? '_self' : '_blank'}`} rel={`${node.data.uri.startsWith("http://localhost:8000") ? '' : 'noopener noreferrer'}`}>{node.content[0].value}</a>;
        }
      }
    }
    return (
